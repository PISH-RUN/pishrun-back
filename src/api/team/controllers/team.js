  ...
  ({ strapi }) => ({
    ...
    adminTeams: async (ctx) => {
      ...
      let teams = await strapi.db.query("api::team.team").findMany({
        ...
        populate: ["participants", "participants.tasks", "participants.tasks.specialty", "tasks.required_prerequisites", "tasks.prerequisites", "participants.users_permissions_user", "tasks", "tasks.link", "tasks.specialty", "tasks.participant", "tasks.files"]
      });
